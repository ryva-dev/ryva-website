...
function state(overrides = {}) {
  ...
  if (base.portfolio?.condition === "strong" && !(base.portfolio.evidence || []).length) {
    base.portfolio.evidence = ["validated as current, niche-relevant, and commercially credible for this scenario"];
    base.portfolio.lastValidatedAt ||= "2026-07-12";
  }
  if (base.portfolio?.condition === "strong" && base.readiness?.level === "unknown") {
    base.readiness = { level: "outreach_ready", blockers: [], confidence: .9 };
  }
  return base;
}

export const MARA_PHASE2_SCENARIOS = [
  { id: "no-niche-no-portfolio", name: "No niche and no portfolio", expected: ["clarify_positioning"], avoid: ["strengthen_pipeline"], maxCost: .30,
  ...
  { id: "twenty-unsent", name: "Twenty unsent opportunities", expected: ["reduce_unsent_backlog"], avoid: ["strengthen_pipeline"], maxCost: .08,
    state: state({ niche: "food recipe and CPG product demonstrations", positioning: "Practical weeknight recipes and credible packaged-food demonstrations for busy households", preferences: { brandFit: ["food", "kitchen", "family grocery"], excludedCategories: ["alcohol"] }, portfolio: { condition: "strong", evidence: ["12 current recipe and CPG demonstration samples validated"], lastValidatedAt: "2026-07-12" }, unsentOutreachBacklog: Array.from({ length: 20 }, (_, i) => ({ id: `opp-${i}`, brandName: `Food brand ${i + 1}`, category: i % 2 ? "CPG" : "kitchen", confidence: .5 + i / 50, fitEvidence: "matches recipe or product-demonstration positioning" })), workload: { maraOpen: 20, creatorOpen: 20, ignoredCount: 1, blockedCount: 0 } }) },
  ...
  { id: "limited-time", name: "Creator with very limited time", expected: ["strengthen_pipeline"], avoid: [], maxCost: .15,
    state: state({ niche: "home organization", portfolio: { condition: "strong", lastValidatedAt: "2026-07-12", evidence: ["10 current home-organization samples validated as commercially credible"] }, readiness: { level: "outreach_ready", blockers: [], confidence: .94 }, capacity: { weekdayMinutes: 30, weekendMinutes: 120, availableWindows: ["Sunday 14:00-16:00"], temporaryConstraint: "full-time job and childcare" }, strategyReason: "Creator effort must be batched into one decision and one Sunday production window." }) },
  ...
  { id: "gifted-preference", name: "Gifted-opportunity preferences", expected: ["assess_gifted_opportunity"], avoid: [], maxCost: .12,
    state: state({ niche: "skincare", portfolio: { condition: "weak", evidence: ["two current skincare samples; a credible product-demonstration gap remains"] }, readiness: { level: "beginner", blockers: ["limited_skincare_proof"], confidence: .8 }, capacity: { weekdayMinutes: 45, weekendMinutes: 120, availableWindows: ["Saturday morning"] }, preferences: { gifted: "selective when portfolio value is meaningful", primaryGoal: "paid work" }, giftedOpportunity: { productValue: 120, cashValue: 0, deliverables: 2, usageRights: "unclear", portfolioFit: "potentially strong" } }) },
  ...
  { id: "ignored-tasks", name: "Repeatedly ignored tasks", expected: ["throttle_and_reenter"], avoid: ["strengthen_pipeline"], maxCost: .03,
    state: state({ niche: "wellness", portfolio: { condition: "strong", evidence: ["validated wellness short-form portfolio"], lastValidatedAt: "2026-07-12" }, readiness: { level: "outreach_ready", blockers: [], confidence: .92 }, preferences: { primaryGoal: "paid wellness partnerships" }, inactive: true, inactivity: { daysSinceReview: 7 }, workload: { maraOpen: 8, creatorOpen: 5, ignoredCount: 5, blockedCount: 0 } }) },
  { id: "international-multilingual", name: "International and multilingual creator", expected: ["assess_international_fit"], avoid: [], maxCost: .25,
    state: state({ niche: "travel", portfolio: { condition: "strong", evidence: ["validated bilingual travel portfolio with Portuguese and English work"], lastValidatedAt: "2026-07-12" }, readiness: { level: "outreach_ready", blockers: [], confidence: .94 }, performance: { pitchesSent: 4, replies: 1, bounceRate: 0, responseRate: .25, contentSignals: [{ audienceMarkets: ["Portugal", "Germany", "EU"], languages: ["Portuguese", "English"] }] }, geographyAndLanguages: { country: "Portugal", languages: ["Portuguese", "English"], paymentCurrencies: ["EUR"], shippingMarkets: ["EU"] }, internationalOpportunity: { market: "Germany", language: "English", shipsTo: ["EU"], paysIn: "EUR", regulatoryStatus: "requires confirmation", audienceFit: "supported by existing German audience signal" } }) },
  ...
  { id: "overdue-payment", name: "Overdue payment", expected: ["resolve_overdue_payment"], avoid: ["strengthen_pipeline"], maxCost: .06,
    state: state({ niche: "beauty", portfolio: { condition: "strong" }, activeOpportunities: [{ id: "deal-paid-work", stage: "delivered", clientName: "Beauty Client" }], revenue: { expected: 800, confirmed: 800, paid: 0, invoicesDue: [], invoicesOverdue: [{ id: "inv-17", amount: 800, daysOverdue: 7, dispute: false, clientName: "Beauty Client", contactChannel: "email", invoiceSentTo: "accounts payable contact on file" }] } }) },
