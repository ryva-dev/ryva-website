...
} from "../redesign/relationship/RelationshipDetail";
...
type Source = { id: string; reference: string; status?: string; capturedAt?: string };
type ContactContext = {
  record: Row;
  related: Row[];
  evidence: Row[];
...
function dateTime(value: unknown, fallback = "Time not recorded"): string {
  return typeof value === "string" && value ? new Date(value).toLocaleString() : fallback;
}

function dateTimeInput(value: string | Date = new Date()): string {
...
export function ContactIntelligencePage() {
  ...
  const loadingTrail = <RelationshipTrail items={[{ label: "Contacts", to: "/records/contact" }, { label: loading ? "Loading Contact" : "Contact unavailable" }]} />;
  if (!context && loading) {
    return <div className="page ry-relationship-page">{loadingTrail}<IdentityHeader eyebrow="Contact relationship" title="Loading Contact" status={<StatusLabel value="loading" />} /><LoadingState label="Loading Contact relationship" /></div>;
  }
  if (!context) {
    return <div className="page ry-relationship-page">{loadingTrail}<IdentityHeader eyebrow="Contact relationship" title="Contact unavailable" /><ErrorState message={loadError} action={<Button variant="secondary" onClick={() => void load()}>Try again</Button>} /></div>;
  }
  ...
  const contextContent = (
    <>
      ...
      <div className="ry-context-item">
        ...
        <p>{context.tasks.filter((task) => task.status !== "completed").length} open Tasks</p>
  ...
  return (
    <div className="page ry-relationship-page">
      <RelationshipTrail items={[
        { label: "Contacts", to: "/records/contact" },
